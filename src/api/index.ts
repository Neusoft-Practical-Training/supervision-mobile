...
export const getConfirmHistory = async (data: {
  gm_id: number,
  confirmHistoryFilterCriteria: ConfirmHistoryFilterCriteria
}) =>
  (await instance.post<AqiAssignment[]>("/getConfirmHistory", data)).data;

export const getFeedbackHistory = async (data: {
  supervisor_id: number,
  feedbackHistoryFilterCriteria: FeedbackHistoryFilterCriteria
}) =>
...
export const leaveRequest = async (request: LeaveRequestDTO) =>
  (await instance.post<LeaveRequestDTO>("/leaveRequest", request)).data;

export const getLeaveRequest = async (gridMemberId: number) =>
  (await instance.get<LeaveRequestDTO>(`/getLeaveRequest/${gridMemberId}`)).data;