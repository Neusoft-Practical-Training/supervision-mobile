  ...
  watch: {
    ...
    marks: {
      handler(newMarks: Mark[], oldMarks: Mark[]) {
        const newMarksLength = newMarks.length;
        const oldMarksLength = oldMarks.length;
        if (newMarksLength > oldMarksLength) {
          // 计算新增的标记
          const addedMarks = newMarks.slice(oldMarksLength, newMarksLength);

          // 添加新增的标记
          addedMarks.forEach(mark => this.addMark(mark));
        } else {
          for (let i = newMarksLength; i < oldMarksLength; i++) {
            this.removeMark(i);
          }
        }
  ...
  methods: {
    ...
    clearMarks() {
      if (this.markers && this.markers.length) {
        this.markers.forEach(marker => {
          this.mapComponent.remove(marker);
          marker.setMap(null);
        });
        this.markers = [];
      }
    ...
    removeMark(index: number) {
      if (this.markers && this.markers[index]) {
        this.mapComponent.remove(this.markers[index]);
